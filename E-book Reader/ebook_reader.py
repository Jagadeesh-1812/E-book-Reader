...
from PyQt6.QtGui import QTextCursor, QTextCharFormat, QColor, QIcon, QPalette, QFont, QPixmap, QPainter, QPainterPath
...
LOGO_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ebook logo.png")
...
class LoginWindow(QDialog):
    ...
    def init_ui(self):
        ...
        layout.setContentsMargins(40, 50, 40, 50)  # Increased margins
...
class TextToSpeechThread(QThread):
    # This class is designed to handle text-to-speech operations efficiently by running in its own thread,
    # ensuring the main application doesn’t freeze while speaking. We use the pyttsx3 library to convert 
    # text into speech, and the class provides flexibility with voice selection and speech rate.
    # Additionally, the speech can be stopped at any time if necessary.
    finished = pyqtSignal()
    ...
    def run(self):
        try:
            ...
            if self.voice_id:
                self.engine.setProperty('voice', self.voice_id)   # Set voice if provided
            self.engine.setProperty('rate', self.rate)# Set speech rate
            self.engine.say(self.text) # Queue the text for speech
            self.engine.runAndWait() # Wait for the speech to finish
            if not self.is_stopped:
                self.finished.emit() # Emit finished signal if the operation was successful
        except Exception as e:
            self.error.emit(str(e)) # Emit error signal in case of an exception
            self.finished.emit() # Emit finished signal regardless of error
    
    def stop(self):
        self.is_stopped = True
        if self.engine:
            try:
                self.engine.stop() # Stops the speech immediately
            except:
                pass

class EBookReader(QMainWindow):
    # The EBookReader class is the core of the E-Book Reader application. It creates a window 
    # that welcomes the user by displaying their username in the title. The background is 
# a custom image, and the interface is enhanced with a semi-transparent overlay for 
# better readability. The class also initializes several variables to manage the currently 
# open document, track bookmarks, notes, and highlights, and control the text-to-speech functionality.
    ...
    def __init__(self, username):
        ...
        background_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "back.jpg")
        background_path = background_path.replace('\\', '/')  # Convert to forward slashes for Qt
        ...
        # Create main widget with semi-transparent overlay
        ...
        central_widget.setStyleSheet("""
            QWidget {
                background: qlineargradient(
                    x1: 0, y1: 0,
                    x2: 0, y2: 1,
                    stop: 0 rgba(18, 18, 18, 0.75),
                    stop: 0.3 rgba(18, 18, 18, 0.85),
        ...
        # Main layout
        # This section creates the main layout for the E-Book Reader interface. The main layout contains 
# a welcome message with a logo that is displayed in a circular mask and a personalized greeting 
# with the user's name. A semi-transparent background is applied to the welcome container for better 
# readability. It also initializes a splitter to divide the main window into left and right panels.
        ...
        welcome_container.setStyleSheet("""
            QWidget {
                background: rgba(18, 18, 18, 0.4);  /* More transparent background */
                border-radius: 10px;
                padding: 10px;
            }
        """)
        
        # Add logo with high quality
        logo_label = QLabel()
        logo_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ebook logo.png")
        if os.path.exists(logo_path):
            ...
            x = (size - scaled_pixmap.width()) // 2 if scaled_pixmap.width() < size else 0
            y = (size - scaled_pixmap.height()) // 2 if scaled_pixmap.height() < size else 0
            painter.drawPixmap(x, y, scaled_pixmap)
            painter.end()
            
            logo_label.setPixmap(target)
    ...
    def add_book(self):
        file_path, _ = QFileDialog.getOpenFileName(  # Open a file dialog to select a PDF file
            self, "Open E-Book", "", "PDF Files (*.pdf)"  
        )
        if file_path:
            self.book_list.addItem(os.path.basename(file_path))  # Add the filename (not full path) to the book list
            self.load_book(file_path)  # Load the full PDF file for reading
            
    def load_book(self, item):
         # Check if the input is a direct file path (string)
        if isinstance(item, str):
            file_path = item
        else:
            file_path = item.text() # This assumes the text is the full path, which may not be true
            
        try:
            self.current_doc = fitz.open(file_path)
            self.current_page = 0
            self.display_page()
        except Exception as e:  # Show an error message if the file can't be opened
...
# Launches the login window first, and if login is successful, opens the EBookReader main window with the username.
